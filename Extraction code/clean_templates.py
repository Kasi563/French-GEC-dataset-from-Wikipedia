"""
This python file provides a removed_template function. It cleans some of the french wikipedia templates using shuffle.
"""
...
def get_template_type(template: str):
    ...
    # Siècles modèles 1
    match = re.match(r"-?[Ss](a[vp])?-?( [Mm]ini-?)? ?\|", template)
    if match:
        _, i = match.span()
    ...
    # Siècles modèles 2
    match = re.match(r"-?s(2|p)-? ?\|", template)
    if match:
        _, i = match.span()
    ...
    # Citation
    match = re.match(r"[Cc]ita(tion)? ?( étrangère)?( [Bb]loc)? ?\|", template)
    if not match:
    ...
    # Liens et notes
    match = re.match(r"(([Ll]ien ([Ww]eb)|(brisé))|([Nn]otes?)|([Aa]rticles?)) ?\|", template)
    if match:
        return ""
...
def remove_templates(content: str) -> str:
    """
    Takes a string and cleans the french wiki templates if possible. Returns the cleaned version of the string.
    Arg:
        - content: string to be cleaned
    """
